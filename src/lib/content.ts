...
export function getServicesForBranch(branchSlug: string): readonly Service[] {
  const branch = getBranchBySlug(branchSlug);
  return branch ? bySlugs(services, branch.serviceSlugs) : [];
}

export function getBranchesForService(serviceSlug: string): readonly Branch[] {
  return branches.filter((branch) => branch.serviceSlugs.includes(serviceSlug));
}

export function getOptometrists(): readonly Optometrist[] {
  return optometrists;
}

export function getOptometristsForBranch(branchSlug: string): readonly Optometrist[] {
  const branch = getBranchBySlug(branchSlug);
...
export function getFaqsByCategory(): readonly { category: string; faqs: readonly Faq[] }[] {
...
export function getTopFaqs(limit = 4): readonly Faq[] {
